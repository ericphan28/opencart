...
class Currency extends \Opencart\System\Engine\Controller {
	...
	public function index(): string {
		...
		// Kiểm tra nếu currency không tồn tại, dùng currency mặc định
		if (!isset($data['currencies'][$code])) {
			$code = $this->config->get('config_currency');
		}

		// Nếu vẫn không tồn tại, dùng currency đầu tiên có sẵn
		if (!isset($data['currencies'][$code]) && !empty($data['currencies'])) {
			$code = array_key_first($data['currencies']);
		}

		// Cập nhật session với currency hợp lệ
