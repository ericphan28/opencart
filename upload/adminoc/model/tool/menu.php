...
class Menu extends \Opencart\System\Engine\Model {
	...
	public function getMenus(): array {
		...
		// Lấy language_id hiện tại
		$language_id = $this->config->get('config_language_id');
		if (!$language_id) {
			// Fallback nếu không có config, lấy từ database  
			$language_query = $this->db->query("SELECT language_id FROM `" . DB_PREFIX . "language` WHERE code = '" . $this->db->escape($this->config->get('config_language_admin')) . "'");
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "menu` `m` LEFT JOIN `" . DB_PREFIX . "menu_description` `md` ON (`m`.`menu_id` = `md`.`menu_id` AND `md`.`language_id` = '" . (int)$language_id . "') ORDER BY `m`.`sort_order` ASC");
